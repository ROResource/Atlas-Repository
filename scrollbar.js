(function () {
    ...
    function setupNavButtonEvents() {
      ...
      function startHold(direction) {
        ...
        goToPage(currentPage + direction); // immediate page change
      ...
      prevBtn?.addEventListener('touchstart', () => startHold(-1), { passive: false });
      ...
      nextBtn?.addEventListener('touchstart', () => startHold(1), { passive: false });
    ...
    function setupDragEvents() {
      ...
      track.addEventListener('click', (e) => {
        if (e.target === thumb) return; // Ignore clicks on the thumb
        const left = getThumbLeftFromEvent(e);
        thumb.style.left = `${left}px`;
    ...
    function waitForPDFViewer() {
      const check = setInterval(() => {
        ...
        if (app?.pdfViewer?.pagesCount > 0) {
          ...
          window.addEventListener('resize', () => updateThumbPosition(getPDFApp()?.page || 1));
        }
      }, 100);
    }
